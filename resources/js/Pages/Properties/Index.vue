    ...
    <GuestLayout>
        <div class="bg-emerald-700 text-white">
            <div class="max-w-7xl mx-auto px-4 py-16 text-center">
                <h1 class="text-4xl font-bold mb-4">Find Your Home in Tanzania</h1>
                <p class="text-lg mb-8 text-emerald-100">Browse houses, apartments, land, and commercial properties across the country.</p>
                <div class="flex gap-2 justify-center flex-wrap">
                    ...
                    <button @click="search" class="bg-emerald-900 hover:bg-emerald-950 px-6 py-3 rounded-lg font-semibold transition">
                        Search
                    </button>
                </div>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-4 py-8">
            ...
            <div v-if="properties.data.length === 0" class="text-center py-16 text-gray-500">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                <Link v-for="property in properties.data" :key="property.id" :href="`/properties/${property.id}`"
                    ...
                    <div class="h-48 bg-gray-200 flex items-center justify-center text-gray-400 group-hover:scale-105 transition">
                        {{ property.property_type }}
                    </div>
                    <div class="p-4">
                        <div class="flex justify-between items-start">
                            <h3 class="font-semibold text-lg text-gray-900">{{ property.title || 'Property' }}</h3>
                            <span class="text-emerald-700 font-bold">{{ property.price_label }}</span>
                        </div>
                        <p class="text-gray-500 text-sm mt-1">{{ property.city }}{{ property.region ? ', ' + property.region : '' }}</p>
            ...
            <div class="flex justify-center gap-2 mt-8">
                <Link v-if="properties.prev_page_url" :href="properties.prev_page_url" class="px-4 py-2 bg-gray-200 rounded-lg">← Prev</Link>
